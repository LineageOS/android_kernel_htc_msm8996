...
typedef struct {
	...
	int lower_limit;
	int upper_limit;
} TARGET_HW_WORKAROUND;

#define NUM_OF_TARGET_HW_WORKAROUND 9
static TARGET_HW_WORKAROUND hw_workaround_table [] = {
        {"HT61S01",	40,	87	},
        {"HT61T01",	4,	13	},
        {"HT61X01",	16,	43	},
        {"HT62R01",	1,	21	},
...
#if LASER_STMVL53L0_HW_WORKAROUND
static int toNumber(char input[]) {
	int digit, result = 0;

	for (digit = 0; input[digit] != '\0'; digit++) {
		result = result * 10 + (input[digit] - '0');
	}
	return result;
}

static void Laser_check_hw_workaround(void) {
        const char* cmdline = NULL;
        char* temp_cmdline = NULL;
	int sn = 0, target = 0;

	I("Laser_check_hw_workaround++\n");
			...
			temp_cmdline = strstr(cmdline, hw_workaround_table[target].header);
				...
				if((sn >= hw_workaround_table[target].lower_limit) &&
				   (sn <= hw_workaround_table[target].upper_limit)) {
					g_hw_workaround &= false;
                        		I("Laser_check_hw_workaround: Excepted, hw_workaround = %d\n",
									g_hw_workaround);
					break;
				}
			}
...
{
	VL53L0_Error Status = VL53L0_ERROR_NONE;
#ifndef LASER_USE_INTERRUPT_MODE
...
static irqreturn_t laser_irq_handler(int irq, void *handle)
{
	VL53L0_Error Status = VL53L0_ERROR_NONE;
	VL53L0_RangingMeasurementData_t RangingMeasurementData;
	u32 data[REPORT_EVENT_COMMON_LEN] = {0};
	...
	if (laser_data->enabled && laser_data->startmeasure) {
		Status = VL53L0_GetRangingMeasurementData(&MyDevice, &RangingMeasurementData);
		if (Status == VL53L0_ERROR_NONE) {
...
static int Laser_parse_dt(struct device *dev, struct laser_device_data *pdata)
{
	...
	u32 buf = 0;
        prop = of_find_property(dt, "laser,hw_workaround", NULL);
        if (prop) {
                of_property_read_u32(dt, "laser,hw_workaround", &buf);
                g_hw_workaround = buf;
                I("%s: hw_workaround = %d\n", __func__, g_hw_workaround);
        } else {
                g_hw_workaround = false;
                I("%s: hw_workaround not found", __func__);
        }
#endif
	...
	pdata->laser_irq_gpio = of_get_named_gpio(dt, "laser,intr-gpio", 0);
	if (!gpio_is_valid(pdata->laser_irq_gpio))
...
static int Laser_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(laser_data->laser_irq_gpio)) {
		...
		gpio_direction_input(laser_data->laser_irq_gpio);
	} else {
		E("%s : irq is not valid\n", __func__);
	}
	laser_data->IRQ = gpio_to_irq(laser_data->laser_irq_gpio);
	I("Requesting irq = %d\n", laser_data->IRQ);
	err = request_threaded_irq(laser_data->IRQ, NULL, laser_irq_handler,
			IRQF_TRIGGER_LOW | IRQF_ONESHOT, "laser", laser_data);
	if (err)
