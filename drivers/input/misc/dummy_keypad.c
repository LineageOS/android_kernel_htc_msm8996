...
static int dummy_keypad_init_func(void)
{
	...
	if (dummy_keypad == NULL) {
		printk(KERN_ERR "[KEY][ERR] %s: Can't allocate memory\n", __func__);
		goto kzalloc_fail;
	}
	...
	if (dummy_keypad->input_dev == NULL) {
		printk(KERN_ERR "[KEY][ERR] %s: Failed to allocate input device\n", __func__);
	...
	if (input_register_device(dummy_keypad->input_dev)) {
		printk(KERN_ERR "[KEY][ERR] %s: Unable to register %s input device\n", __func__,
...
static unsigned int support_dummy_keypad = 0;

static int __init dummy_keypad_init(void)
{
	pr_info("[KEY] %s: %s: initial running...\n", __func__, DRIVER_NAME);
	if(support_dummy_keypad == 1)
		return dummy_keypad_init_func();
	else
		return 0;
}

static int __init get_dummy_keypad_support_flag(char *str)
{
	int ret = kstrtouint(str, 0, &support_dummy_keypad);
	pr_info("[KEY] androidkey.dummy %d: %d from %s\r\n",
			ret, support_dummy_keypad, str);
	return ret;
} early_param("androidkey.dummy", get_dummy_keypad_support_flag);

static void __exit dummy_keypad_exit(void)
{
	dummy_keypad_exit_func();
	pr_info("[KEY] %s: %s: exit...\n", __func__, DRIVER_NAME);
}
