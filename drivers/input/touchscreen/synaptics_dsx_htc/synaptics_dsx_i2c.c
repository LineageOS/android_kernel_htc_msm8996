...
#define SYN_I2C_RETRY_RESET_TIMES (SYN_I2C_RETRY_TIMES/2)
#define I2C_ERROR_ARB_LOST -2
#define I2C_ERROR_TIMEOUT -110
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		...
		if ( retval == 2) {
			retval = length;
			break;
		} else if (retval == I2C_ERROR_ARB_LOST || retval == I2C_ERROR_TIMEOUT || retry == SYN_I2C_RETRY_RESET_TIMES)
			synaptics_rmi4_reset();
		dev_dbg(rmi4_data->pdev->dev.parent,
				"%s: I2C retry %d (%d)\n",
