...
static struct htc_35mm_pmic_info *hi = NULL;
...
static int htc_headset_pmic_probe(struct platform_device *pdev)
{
	...
	uint32_t vers = 0;
#endif
	printk(KERN_ERR "htc_headset_pmic_probe ++\n");
//HTC_AUD_START
	if(!get_htc_headset_mgr_status()) {
		return -EPROBE_DEFER;
	}
//HTC_AUD_END
...
//HTC_AUD_START
	if (!(hi->htc_accessory_class)) {
		printk(KERN_ERR "htc_headset_pmic_probe: hi->htc_accessory_class is null\n");
