...
static struct htc_headset_mgr_info *hi = NULL;
...
static int probe_success = 0; //HTC_AUD
...
struct class *hs_get_attribute_class(void)
{
	if (hi)
		...
		printk(KERN_ERR "hs_get_attribute_class: hi is null..\n");
		return NULL;
	}
}
...
int headset_notifier_register(struct headset_notifier *notifier)
{
	...
	case HEADSET_REG_HS_INSERT:
		...
		break;
	case HEADSET_REG_MIC_BIAS2:
		HS_LOG("Register MIC_BIAS2 notifier");
		hs_mgr_notifier.mic_bias2_enable = notifier->func;
		break;
	default:
...
static void headset_power(int enable)
{
	...
	if (hs_mgr_notifier.mic_bias2_enable)
		hs_mgr_notifier.mic_bias2_enable(enable);
	else
		pr_info("%s: mic_bias2_enable is null\n", __func__);
}
...
{
	printk(KERN_ERR "headset_mgr_status: probe_success %d\n", probe_success);
...
static int htc_headset_mgr_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "htc_headset_mgr_probe++\n");
//HTC_AUD_START
