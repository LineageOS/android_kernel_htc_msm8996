...
static struct usb_endpoint_descriptor projector_superspeed_in_desc = {
...
static struct usb_endpoint_descriptor projector_superspeed_out_desc = {
	.bLength                = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType        = USB_DT_ENDPOINT,
...
static struct usb_ss_ep_comp_descriptor projector_superspeed_in_comp_desc = {
	.bLength =		sizeof projector_superspeed_in_comp_desc,
...
static struct usb_ss_ep_comp_descriptor projector_superspeed_out_comp_desc = {
	.bLength =		sizeof projector_superspeed_out_comp_desc,
	.bDescriptorType =	USB_DT_SS_ENDPOINT_COMP,
...
static struct usb_descriptor_header *ss_projector_descs[] = {
	(struct usb_descriptor_header *) &projector_interface_desc,
	(struct usb_descriptor_header *) &projector_superspeed_in_desc,
	(struct usb_descriptor_header *) &projector_superspeed_in_comp_desc,
	(struct usb_descriptor_header *) &projector_superspeed_out_desc,
	(struct usb_descriptor_header *) &projector_superspeed_out_comp_desc,
	NULL,
};
...
int htc_usb_enable_function(char *name, int ebl); /*++ 2015/11/11 USB Team, PCN00037 ++*/
static void usb_setup_android_projector(struct work_struct *work)
{

	if (projector_dev) {
		htc_usb_enable_function("adb,mass_storage,serial,projector", 1); /*++ 2015/11/11 USB Team, PCN00037 ++*/
	...
	int iPenType, int iX, int iY)
{
	...
	if (iPenType != 3) {
		if (b_firstPenDown) {
			...
			pr_info("projector: Pen down %d, %d\n", iX, iY);
		} else {
			...
			if (iX != iCal_LastX || iY != iCal_LastY) {
				...
				iReportCount++;
				if (iReportCount < 10)
					pr_info("projector: Pen move %d, %d\n", iX, iY);
			}
		}
...
static struct size get_projection_size(struct projector_dev *dev, struct msm_client_info *client_info)
{
	...
	pr_info("perserve_aspect_ratio= %d\n", perserve_aspect_ratio);
	...
	pr_info("projector size(w=%d, h=%d)\n", ret.w, ret.h);
		...
		if (!strncmp("init", data, 4)) {
			...
			queue_work(dev->wq_display, &dev->send_fb_work_legacy);
			 ...
			 if (mouse_data[0] < 4) {
				...
				pr_info("projector: Key command data %02x, keycode %d\n",
					*((char *)(req->buf)), mouse_data[0]);
			}
		} else if (mouse_data[0] != 0)
			pr_err("projector: Unknow command data %02x, mouse %d,%d,%d\n",
				...
				struct usb_endpoint_descriptor *out_desc)
{
	...
	pr_debug("projector_create_bulk_endpoints dev: %p\n", dev);
	...
	pr_debug("usb_ep_autoconfig for projector ep_out got %s\n", ep->name);
	ep->driver_data = dev;		/* claim the endpoint */
	dev->ep_out = ep;
...
fail:
	...
	pr_err("projector: could not allocate requests\n");
	return -1;
}
...
static int projector_bind_config(struct usb_configuration *c)
{
	...
	if (gadget_is_superspeed(c->cdev->gadget))
		dev->function.ss_descriptors = ss_projector_descs;
...
err_free:
	pr_err("projector gadget driver failed to initialize, err=%d\n", ret);
	return ret;
}
...
static void projector_complete_req(struct usb_ep *ep, struct usb_request *req)
{
	...
	pr_info("status=%d, request=0x%02X\n",
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR) {
		...
		pr_info("request(req=0x%02x, wValue=%d, "
						 "wIndex=%d, wLength=%d)\n", 
		...
		if (ctrl->bRequest == HTC_MODE_CONTROL_REQ) {
			...
			case HSML_06_REQ_GET_SERVER_NONCE:
				if (w_length == HSML_SERVER_NONCE_SIZE) {
					...
					pr_err("The request size for server nonce is incorrect. (w_length=%d)", w_length);
				value = w_length;
			...
			default:
				pr_info("unrecognized request(req=0x%02x, wValue=%d, "
								 "wIndex=%d, wLength=%d)\n",
						ctrl->bRequest, ctrl->wValue, ctrl->wIndex, ctrl->wLength);
				break;
			}
