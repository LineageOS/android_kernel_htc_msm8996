	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR) {
		...
		if (prj2_dev->hsml_ver == cHSML_VER_12)
		{
            switch (ctrl->bRequest) {
                ...
                default:
                    pr_info("unrecognized request (HSML_12) (req=0x%02x, wValue=%d, "
                                     "wIndex=%d, wLength=%d)\n",
                            ctrl->bRequest, ctrl->wValue, ctrl->wIndex, ctrl->wLength);
                    break;
            }
...
#endif
		{
			...
			for (i = 0; i <= 26; i++) {
                ...
                default:
                    pr_info("unrecognized request (HSML_08) (req=0x%02x, wValue=%d, "
                                     "wIndex=%d, wLength=%d)\n",
                            ctrl->bRequest, ctrl->wValue, ctrl->wIndex, ctrl->wLength);
                    break;
            }
...
u8 ucTmpUUID[] = {
	...
	if (ret < 0) {
		pr_err("usb prj2_status_sdev switch_dev_register register fail\n");
		goto err_free;
	}
