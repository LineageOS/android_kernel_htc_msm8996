...
#define USBC_DEV_NAME "utc_fwu"
#define USBC_LOG_NAME "[USBC][FWU]"

#ifdef pr_fmt
#undef pr_fmt
#endif
#define pr_fmt(fmt) USBC_LOG_NAME ": " fmt
...
#undef pr_err
#endif
#define pr_err(fmt, ...) printk(KERN_ERR USBC_LOG_NAME \
                       "[ERR]:(%s:%d): " fmt "\n", __func__, __LINE__, ##__VA_ARGS__)

#define USBC_FWU_IOCTL_CODE			(0x85)
#define FW_UPDATE_PROCCESS			_IO(USBC_FWU_IOCTL_CODE, 1)
#define FW_FILE_SIZE				_IOW(USBC_FWU_IOCTL_CODE, 2, uint32_t)
#define FW_FILE_REQUEST				_IO(USBC_FWU_IOCTL_CODE, 3)
#define FW_LOAD_DONE				_IO(USBC_FWU_IOCTL_CODE, 4)
#define FW_UPDATE_BYPASS			_IO(USBC_FWU_IOCTL_CODE, 5)
...
static ssize_t usbc_vendor_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
...
static ssize_t usbc_fw_ver_show(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	struct data *fwu_data = dev_get_drvdata(dev);
...
static struct device_attribute dev_attr[] = {
	...
	__ATTR(vendor, S_IRUGO, usbc_vendor_show, NULL),
	__ATTR(fw_ver, S_IRUGO, usbc_fw_ver_show, NULL),
...
static int usb_typec_fwu_open(struct inode *inode, struct file *filp)
{
...
static int usb_typec_fwu_release(struct inode *inode, struct file *filp)
{
	struct data *fwu_data = container_of(inode->i_cdev, struct data, fwu_cdev);
...
static ssize_t usb_typec_fwu_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
...
static unsigned int usb_typec_fwu_poll(struct file *filp, struct poll_table_struct *wait)
{
	return 0;
}

static long usb_typec_fwu_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
	int ret = 0;
	u32 data = 0;
	struct cdev_data *fw_cdev = filp->private_data;
	...
	switch (cmd) {
		case FW_UPDATE_PROCCESS:
			...
			ret = usb_typec_fw_update_process(fwu_data, fw);
			if (ret == 1) {
...
#ifdef CONFIG_COMPAT
static long
usb_typec_fwu_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	return usb_typec_fwu_ioctl(filp, cmd, (unsigned long)compat_ptr(arg));
}
#else
...
static int usb_typec_fw_update_process(struct data *fwu_data, struct firmware *fw)
{
	struct usb_typec_fwu_notifier *notifier = fwu_data->notifier;
...
EXPORT_SYMBOL(usb_typec_fw_update_progress);

static const struct file_operations usb_typec_fwu_fops = {
	.owner  = THIS_MODULE,
	.read   = usb_typec_fwu_read,
	.write  = usb_typec_fwu_write,
	.unlocked_ioctl = usb_typec_fwu_ioctl,
	.compat_ioctl = usb_typec_fwu_compat_ioctl,
	.open   = usb_typec_fwu_open,
	.release= usb_typec_fwu_release,
	.poll   = usb_typec_fwu_poll,
};
...
int register_usbc_fw_update(struct usb_typec_fwu_notifier *notifier)
{
	...
	if (major_num) {
		...
		ret = register_chrdev_region(dev_no, 1, USBC_DEV_NAME);
	} else {
		ret = alloc_chrdev_region(&dev_no, 0, 1, USBC_DEV_NAME);
		if (ret < 0) {
	...
	cdev_init(&fwu_data->fwu_cdev, &usb_typec_fwu_fops);
	...
	fwu_data->fwu_class = class_create(THIS_MODULE, USBC_DEV_NAME);
	...
	snprintf(dev_name, 13, "%s", USBC_DEV_NAME);
	...
	ret = sysfs_create_link(NULL, &fwu_data->fwu_dev->kobj, "android_usbc_fwu");
	if (ret < 0) {
...
EXPORT_SYMBOL(register_usbc_fw_update);
...
void unregister_usbc_fw_update(struct usb_typec_fwu_notifier *notifier)
{
	...
	sysfs_remove_link(NULL, "android_usbc_fwu");
	for (attr_count = 0; attr_count < ARRAY_SIZE(dev_attr); attr_count++) {
		sysfs_remove_file(&fwu_data->fwu_dev->kobj, &dev_attr[attr_count].attr);
	}
	device_unregister(fwu_data->fwu_dev);
...
EXPORT_SYMBOL(unregister_usbc_fw_update);

MODULE_AUTHOR("HTC");
MODULE_DESCRIPTION("usb_typec Firmware update Driver");
