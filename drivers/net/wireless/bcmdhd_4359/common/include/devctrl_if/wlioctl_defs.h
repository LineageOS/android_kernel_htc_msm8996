...
#define WL_CHAN_RADAR		(1 << 3)	/* indicates if the channel is radar sensitive in
						 * the current country; (requires CAC and ISM)
						 */
#define WL_CHAN_INACTIVE	(1 << 4)	/* temporarily inactive due to radar */
#define WL_CHAN_PASSIVE		(1 << 5)	/* channel is in passive mode */
#define WL_CHAN_RESTRICTED	(1 << 6)	/* restricted use channel */
#define WL_CHAN_WEATHER_RADAR	(1 << 7)	/* indicates if the channel has weather radar
						 * requirement in the current country; (requires
						 * longer CAC)
						 */
