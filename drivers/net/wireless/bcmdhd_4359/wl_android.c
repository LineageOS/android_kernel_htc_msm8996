 ...
 *
 * $Id: wl_android.c 619124 2016-02-15 09:26:47Z $
 */
...
static int wl_android_ltecoex_mode(struct net_device *dev, char *command, int total_len);
...
#define TRAFFIC_ULTRA_HIGH_WATER_MARK	23000
#define TRAFFIC_SUPER_HIGH_WATER_MARK	12000
...
extern int dhd_wk_lock_stats_dump(dhd_pub_t *dhdp, char *buf, ssize_t len);
#endif 
...
int wl_android_wifi_on(struct net_device *dev)
{
	...
	if (!g_wifi_on) {
		...
		dhd_set_perf_state(dev, TRAFFIC_STATS_NORMAL, TRUE);
#endif
	}
...
int wl_android_wifi_off(struct net_device *dev, bool on_failure)
{
	...
	if (dhd_APUP) {
		...
		if (check_hang_already(dev)) {
			...
			DHD_ERROR(("Marked wait for this feautre done\n"));
			wl_cfg80211_send_priv_event(dev, "AP_DOWN");
		}
	}
#endif 
	...
	wlan_unlock_perf();
	wlan_lock_irq_affinity(dev, 0);
	dhd_lb_affinity_enable(dev, TRAFFIC_STATS_NORMAL);
#ifdef DHD_TRACE_PERF_STATE
	dhd_set_perf_state(dev, TRAFFIC_STATS_NORMAL, TRUE);
#endif
	bcm_mdelay(500);
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		...
		char *rev_info_delim = country_code + 2; 
		int revinfo = -1;
		if ((rev_info_delim) &&
		...
		dhd_pub_t *dhdp = &(dhd->pub);
#endif 
		bytes_written = dhd_wk_lock_stats_dump(dhdp, command, priv_cmd.total_len);
	}
#endif 
#ifdef CUSTOMER_HW_ONE
#if defined(DHD_DEBUG) && defined(DHD_FW_COREDUMP)
		...
		dhdpcie_runtime_bus_wake(dhdp, TRUE, wl_android_priv_cmd);
#endif 
		DHD_OS_WAKE_LOCK(dhdp);
		DHD_ERROR(("%s CMD_TRIG_RAMDUMP \n", __FUNCTION__));
		dhdp->memdump_type = DUMP_TYPE_USER_TRIGGER;
...
static int wl_android_auto_channel(struct net_device *dev, char *command, int total_len)
{
	...
	if (res) {
		DHD_ERROR(("%s fail to set spect 0\n", __func__));
		goto fail;
	}
	...
	if ((current_traffic_count >= last_traffic_count &&
		...
		case TRAFFIC_STATS_NORMAL:
			if (traffic_diff > TRAFFIC_HIGH_WATER_MARK) {
				...
				if (traffic_diff > TRAFFIC_SUPER_HIGH_WATER_MARK) {
					...
					dhd_lb_affinity_enable(dev, traffic_stats_flag);
					DHD_ERROR(("lock 2nd cpu here, traffic-count=%ld\n",
						traffic_diff));
					if (traffic_diff > TRAFFIC_ULTRA_HIGH_WATER_MARK) {
						traffic_stats_flag = TRAFFIC_STATS_ULTRA_HIGH;
						wlan_lock_irq_affinity(dev, 1);
						dhd_lb_affinity_enable(dev, traffic_stats_flag);
						DHD_ERROR(("lock irq affinity here, traffic-count=%ld\n", traffic_diff));
					}
				}
			}
			break;
		...
		case TRAFFIC_STATS_SUPER_HIGH:
			if (traffic_diff > TRAFFIC_ULTRA_HIGH_WATER_MARK) {
				...
				dhd_lb_affinity_enable(dev, traffic_stats_flag);
				DHD_ERROR(("lock irq affinity here, traffic-count=%ld\n", traffic_diff));
			} else if (traffic_diff < TRAFFIC_SUPER_HIGH_WATER_MARK) {
				traffic_stats_flag = TRAFFIC_STATS_HIGH;
				wlan_unlock_multi_core(dev);
				dhd_lb_affinity_enable(dev, traffic_stats_flag);
			...
			break;
		case TRAFFIC_STATS_ULTRA_HIGH:
			if (traffic_diff < TRAFFIC_ULTRA_HIGH_WATER_MARK) {
				traffic_stats_flag = TRAFFIC_STATS_SUPER_HIGH;
				wlan_lock_irq_affinity(dev, 0);
				dhd_lb_affinity_enable(dev, traffic_stats_flag);
				DHD_ERROR(("unlock irq affinity here, traffic-count=%ld\n",
					traffic_diff));
				if (traffic_diff < TRAFFIC_SUPER_HIGH_WATER_MARK) {
					traffic_stats_flag = TRAFFIC_STATS_HIGH;
					wlan_unlock_multi_core(dev);
...
void wlan_lock_multi_core(struct net_device *dev)
{
	...
	snprintf(buf, sizeof(buf), "PERF_LOCK freq=high cpu=%u", (nr_cpu_ids <= 2) ? nr_cpu_ids : 2);
...
void wlan_lock_irq_affinity(struct net_device *dev, int set)
{
...
wl_android_get_assoc_sta_list(struct net_device *dev, char *buf, int len)
{
	...
	bcm_mdelay(500);
	maclist->count = MAX_ASSOC_NUMBER;
...
static int wl_android_set_ap_mac_list(struct net_device *dev, void *buf)
{
	...
	if (white_maclist->count > MACLIST_MAX_NUMBER ||
		black_maclist->count > MACLIST_MAX_NUMBER) {
	...
	if (mac_mode == MACLIST_MODE_DISABLED) {
		...
		for (i = 0; (i < android_ap_white_list.count) && (i < MACLIST_MAX_NUMBER); i++)
		...
		for (i = 0; (i < android_ap_black_list.count) && (i < MACLIST_MAX_NUMBER); i++)
		...
		assoc_maclist->count = MAX_ASSOC_NUMBER;
		wldev_ioctl(dev, WLC_GET_ASSOCLIST, assoc_maclist, 256, 0);
		if (assoc_maclist->count) {
			uint j;
			for (i = 0; (i < assoc_maclist->count) && (i < MAX_ASSOC_NUMBER); i++) {
				for (j = 0; (j < android_ap_white_list.count) &&
					(j < MACLIST_MAX_NUMBER); j++) {
