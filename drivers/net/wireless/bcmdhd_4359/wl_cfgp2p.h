 ...
 *
 * $Id: wl_cfgp2p.h 578000 2015-08-10 03:18:38Z $
 */
...
#ifdef CFGP2P_ERROR_TEXT
#undef CFGP2P_ERROR_TEXT
#define CFGP2P_ERROR_TEXT		"[WLAN] CFGP2P-LOG) "
#endif
#define CFGP2P_INFORM_TEXT		"[WLAN] CFGP2P-INFO) "
#define CFGP2P_DEBUG_TEXT		"[WLAN] CFGP2P-DEBUG) "
#define CFGP2P_ACTION_TEXT		"[WLAN] CFGP2P-ACTION) "
#else
#define CFGP2P_INFORM_TEXT		"CFGP2P-INFO) "
...
#define	CFGP2P_INFO(args)									\
	do {										\
		if (wl_dbg_level & WL_DBG_INFO) {				\
			printk(KERN_INFO CFGP2P_INFORM_TEXT "%s : ", __func__);	\
			printk args;						\
		}									\
	} while (0)
#define	CFGP2P_DBG(args)								\
	do {									\
		if (wl_dbg_level & WL_DBG_DBG) {			\
			printk(KERN_DEBUG CFGP2P_DEBUG_TEXT "%s :", __func__);	\
			printk args;							\
		}									\
	} while (0)
...
#define	CFGP2P_ACTION(args)								\
	do {									\
		if (wl_dbg_level & WL_DBG_P2P_ACTION) {			\
			printk(KERN_DEBUG CFGP2P_ACTION_TEXT "%s :", __func__);	\
			printk args;							\
		}									\
	} while (0)
